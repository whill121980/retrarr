#!/bin/zsh
# retrarr-wsl.sh — Retro Retriever v0.2.1
...
init_static_globals () {
  typeset -g PLATFORM_FLAVOR="wsl-retrobat"
  # RetroBat lives on the Windows C: drive, mounted in WSL at /mnt/c
  typeset -g RETROBAT_ROOT="${RETROBAT_ROOT:-/mnt/c/RetroBat}"
  if [[ ! -d "${RETROBAT_ROOT}/roms" ]]; then
    for cand in "/mnt/c/RetroBat" "/mnt/c/retrobat"; do
      if [[ -d "${cand}/roms" ]]; then
        RETROBAT_ROOT="${cand}"
        break
      fi
    ...
    # typeset -gr NEOGEO_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/games/NEOGEO"
    ...
    typeset -gr TG16CD_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/pcenginecd"
    ...
    typeset -gr MCD_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/megacd"
    ...
    typeset -gr SS_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/saturn"
    ...
    typeset -gr PSXUS_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/psx"
    ...
    typeset -gr PSXEU_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/psx"
    ...
    typeset -gr PSXJP_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/psx"
    ...
    typeset -gr PSXJP2_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/psx"
    ...
    typeset -gr PSXMISC_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/psx"
    ...
    # typeset -gr AO486_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/dos"
    ...
    typeset -gr CD32_GAMEDIR_DEFAULT="${RETROBAT_ROOT}/roms/amigacd32"
}
...
save_settings () {
    ...
    lines=(
        ...
        "# Generated by retrarr-wsl.sh — edit carefully or use the Settings menu"
        ""
...
disabled_ao486_setnames_all () {
    local -a mgls=("${RETROBAT_ROOT}/_DOS Games"/*.mgl)
...
# Destination: ${RETROBAT_ROOT}/bios/
...
bios_download () {
    local bios_dir="${RETROBAT_ROOT}/bios"
...
# ─── ZAPAROO MODE ──────────────────────────────────────────────────────────────
# Called via: retrarr-wsl.sh --zaparoo CORE "game search term"
...
zaparoo_mode () {
    ...
    if [[ -z $IA_EMAIL && -z $IA_PASS ]]; then
        if [[ ! -f ~/.config/internetarchive/ia.ini && \
            ...
            print -u2 "ERROR: archive.org credentials not configured. Run retrarr-wsl.sh first to set up."
            return 1
        fi
    fi
    if [[ -z $IA ]]; then
        log_error "zaparoo_mode: internetarchive CLI not installed"
        print -u2 "ERROR: internetarchive CLI not installed. Run retrarr-wsl.sh first to set up."
        return 1
    fi
...
main () {
    ...
    # CLI modes
    [[ $* == "${RETROBAT_ROOT}/_DOS Games" ]] && { disabled_ao486_setnames_all ; return }
    [[ -n $* && -d $* ]] && { disabled_organise_chd_dir $* ; return }
    if [[ $1 == "--zaparoo" ]]; then
        [[ -z $2 || -z $3 ]] && { print -u2 "Usage: retrarr-wsl.sh --zaparoo CORE \"game name\"" ; return 1 }
        zaparoo_mode "$2" "$3"
        return $?
    fi
